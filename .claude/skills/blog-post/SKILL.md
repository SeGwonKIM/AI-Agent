...
**강의는 평일에만 열린다 — 주말·공휴일은 건너뛴다.** 날짜를 그대로 믿지 말고 요일을 확인한다.

```bash
date -d 2026-09-07 '+%a'
```

토·일이거나 `강의노트/holidays.txt` 에 있는 날짜면 **폴더 이름이 잘못된 것**이다. 실제로 `0904`(금)
다음을 `0905` 로 저장했는데 그날이 토요일이라, 요약과 블로그 글까지 만든 뒤 전부 되돌린 일이
있었다. 글 주소에 날짜가 박히므로 **올린 뒤에 고치면 주소가 바뀌어 기존 링크가 깨진다.**
